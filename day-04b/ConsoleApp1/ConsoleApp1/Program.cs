...
var moveIdx = 0;
var prevRoundBingoCards = new List<BingoCard>();
while (bingoCards.Any())
{
    prevRoundBingoCards = bingoCards.ToList();
    var bingoCardsToRemove = new List<BingoCard>();
    foreach (var bingoCard in bingoCards)
    {
        bingoCard.Scratch(moves[moveIdx]);
        if (bingoCard.HasBingo())
        {
            bingoCardsToRemove.Add(bingoCard);
        }
    }

    foreach (var bingoCardToRemove in bingoCardsToRemove)
    {
        bingoCards.Remove(bingoCardToRemove);
    }

    moveIdx++;
}

var losingBingoCard = prevRoundBingoCards.First();
var score = losingBingoCard.CalculateScore();
