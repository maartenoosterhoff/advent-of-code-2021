...
public class Line
{
    ...
    public IEnumerable<Point> GetPoints()
    {
        ...
        var xdelta = End.X - Begin.X;
        var ydelta = End.Y - Begin.Y;
        if (Math.Abs(xdelta) == Math.Abs(ydelta))
        {
            // Diagonal, 45 degrees angle
            var xstep = Math.Sign(xdelta);
            var ystep = Math.Sign(ydelta);
            var totalSteps = xdelta * xstep;

            for (var step = 0; step <= totalSteps; step++)
            {
                yield return new Point(Begin.X + step * xstep, Begin.Y + step * ystep);
            }
        }
    }
}
